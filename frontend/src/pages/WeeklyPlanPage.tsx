...
import { lifeBlockCategoryConfig } from "./LifeBlocksPage"
...
function DayColumn({
  ...
  return (
    <div
      className={cn(
        "flex flex-col rounded-2xl border bg-muted/20 p-3",
    ...
    >
      ...
      {planBlocks.length > 0 && (
        <div className="mt-3 border-t border-dashed border-violet-300/60 pt-3">
          <p className="mb-1.5 flex items-center gap-1 text-[10px] uppercase tracking-wide text-violet-700">
  ...
  return (
    <span
      className={cn(
        ...
        isAi ? "bg-violet-100 text-violet-800" : "bg-slate-200 text-slate-700",
      )}
...
function PlanBlockChip({ block }: { block: PlanBlock }) {
  ...
  const tone =
    block.type === "habit"
      ? "border-emerald-300/60 bg-emerald-50/70 text-emerald-900"
      : "border-violet-300/60 bg-violet-50/70 text-violet-900"
  return (
        ...
        <div className="relative" ref={menuRef}>
          ...
          {menuOpen && (
            <div className="absolute right-0 top-full z-10 mt-1 w-40 overflow-hidden rounded-lg border border-border bg-card shadow-lg">
              ...
              <button
                type="button"
                className="flex w-full items-center gap-2 border-t border-border/60 px-3 py-2 text-left text-xs text-red-600 hover:bg-red-50"
                onClick={() => {
                  setMenuOpen(false)
                  onDelete()
                }}
